...
from crewai_tools import SerperDevTool, ScrapeWebsiteTool  # <-- Add ScrapeWebsiteTool if needed
...
# Updated LLM initialization with active Llama 3 70B model
llm = ChatGroq(
    model="llama3-70b-8192",  # Active equivalent model
    temperature=0,
    max_tokens=500,
    timeout=None,
    max_retries=2,
...
def main():
    # ✅ Step 1: Use the new Chitwan brochure PDF
    pdf_path = "chitwan.pdf"

    # ✅ Step 2: Initialize vector database from the new brochure
    print("Setting up vector database...")
    vector_db = setup_vector_db(pdf_path)

    # ✅ Step 3: Provide a better context keyword instead of empty string
    local_context = get_local_content(vector_db, "Chitwan travel information")

    # ✅ Step 4: Ask a relevant question
    query = "What is the best time to visit Chitwan National Park and what animals can I expect to see?"
